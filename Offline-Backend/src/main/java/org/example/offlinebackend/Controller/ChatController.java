...
public class ChatController {
    ...
    @PostMapping("api/chat")
     public ResponseEntity<ChatResponse> sendMessage(@RequestBody Chat chat) {
        System.out.println("Hello World");
        return ResponseEntity.ok(chatService.ChatHandel(chat));
    }
    ...
    @GetMapping("/token/receiver/success")
    public List<PaymentTokenSuccess> tokenReceiverSuccess(@RequestParam String phoneNo) {
        return tokenSuccessRepo.findByReceiverMobile(phoneNo);
    }
    @GetMapping("/token/receiver/failure")
    public List<PaymentTokenFailed> tokenReceiverFailure(@RequestParam String phoneNo) {
        return tokenFailedRepo.findByReceiverMobile(phoneNo);
    }
}
